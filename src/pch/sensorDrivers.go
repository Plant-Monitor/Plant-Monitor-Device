...
func getRawRead_TCS34725() []byte{
	// Connect to the AHT20 sensor using the I2C address
	dev := &i2c.Dev{Addr: tcs34725Address, Bus: bus}
	...
	intensity_low, err := readData(dev, DataLow)
	if err != nil {
		log.Fatal(err)
	}
	intensity_high := readDataOrDie(dev, DataHigh)
	data := [2]byte{intensity_low[0], intensity_high[0]}
